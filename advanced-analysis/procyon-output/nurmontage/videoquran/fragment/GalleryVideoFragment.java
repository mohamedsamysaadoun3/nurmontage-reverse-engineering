...
public class GalleryVideoFragment extends Fragment
{
    ...
    public GalleryVideoFragment(final List gallerySelecteds, final File folder, final GalleryPickerVideo$IPicker iPicker) {
        this.iPicker = iPicker;
        this.folder = folder;
        this.gallerySelecteds = gallerySelecteds;
    }
    
    public static GalleryVideoFragment get(final List list, final File file, final GalleryPickerVideo$IPicker galleryPickerVideo$IPicker) {
        final Class<GalleryVideoFragment> clazz = GalleryVideoFragment.class;
        monitorenter(clazz);
        final Class<GalleryVideoFragment> clazz2 = GalleryVideoFragment.class;
        try {
            synchronized (clazz2) {
                if (GalleryVideoFragment.instance == null) {
                    GalleryVideoFragment.instance = new GalleryVideoFragment(list, file, galleryPickerVideo$IPicker);
                }
                return GalleryVideoFragment.instance;
            }
        }
        finally {
            monitorexit(clazz);
    ...
    private void loadVideos(final View view) {
        ...
        if (folder != null) {
            ...
            new Thread(new GalleryVideoFragment$1(this)).start();
        }
    }
    ...
    public void onDestroyView() {
        ...
        GalleryVideoFragment.instance = null;
        super.onDestroyView();
    }
}
