...
public class VolumeFragment extends Fragment
{
    ...
    public VolumeFragment(final EditMediaFragment$IEditMediaCallback iVolumeCallback, final EntityAudio entityAudio) {
        this.iVolumeCallback = iVolumeCallback;
        this.entityAudio = entityAudio;
    }
    
    private void applyVolume(final boolean b) {
        final float volume = this.volumeSeekBar.getProgress() / 100.0f;
        ...
        if (iVolumeCallback != null) {
            if (b) {
                iVolumeCallback.updateEntity(EffectAudioType.VOLUME, this.entityAudio);
    ...
    public static VolumeFragment getInstance(final EditMediaFragment$IEditMediaCallback editMediaFragment$IEditMediaCallback, final EntityAudio entityAudio) {
        if (VolumeFragment.instance == null) {
            VolumeFragment.instance = new VolumeFragment(editMediaFragment$IEditMediaCallback, entityAudio);
        }
        return VolumeFragment.instance;
    }
    ...
    public View onCreateView(final LayoutInflater layoutInflater, final ViewGroup viewGroup, final Bundle bundle) {
        ...
        final int n = (int)(this.entityAudio.getEffectAudio().getVolume() * 100.0f);
        this.tvProgress.setText((CharSequence)String.valueOf(n));
        (this.volumeSeekBar = (SeekBar)((View)root).findViewById(R$id.volumeSeekBar)).setProgress(n);
        this.volumeSeekBar.setOnSeekBarChangeListener((SeekBar$OnSeekBarChangeListener)new VolumeFragment$1(this));
        ((View)root).findViewById(R$id.btn_done).setOnClickListener((View$OnClickListener)new VolumeFragment$$ExternalSyntheticLambda0(this));
        (this.btnPreview = (ImageButton)((View)root).findViewById(R$id.btn_play)).setOnClickListener((View$OnClickListener)new VolumeFragment$$ExternalSyntheticLambda1(this));
        ((View)root).findViewById(R$id.btn_appl_all).setOnClickListener((View$OnClickListener)new VolumeFragment$$ExternalSyntheticLambda2(this));
        return (View)root;
    }
