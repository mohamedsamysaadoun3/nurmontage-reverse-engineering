...
public class ProVersionActivityDone extends Base implements PurchasesUpdatedListener
{
    ...
    public ProVersionActivityDone() {
        this.color_select = -1;
        this.onBackPressedCallback = new ProVersionActivityDone$1(this, true);
        this.productDetailsMap = new HashMap();
        this.product_id_current = ProVersionActivityDone.PRODUCT_ID_YEAR;
        this.autoScrollHandler = new Handler(Looper.getMainLooper());
        this.isUserScrolling = false;
        this.autoScrollRunnable = new ProVersionActivityDone$10(this);
    }
    ...
    private void checkInAppPurchases() {
        this.billingClient.queryPurchasesAsync(QueryPurchasesParams.newBuilder().setProductType("inapp").build(), (PurchasesResponseListener)new ProVersionActivityDone$$ExternalSyntheticLambda3(this));
    }
    
    private void checkUserSubscriptionStatus() {
        this.billingClient.queryPurchasesAsync(QueryPurchasesParams.newBuilder().setProductType("subs").build(), (PurchasesResponseListener)new ProVersionActivityDone$12(this));
    }
    ...
    private void handleNoPurchases() {
        this.runOnUiThread((Runnable)new ProVersionActivityDone$14(this));
    ...
    private void handlePurchase(final Purchase purchase) {
        ...
        if (purchaseState == (b ? 1 : 0)) {
            if (!purchase.isAcknowledged()) {
                this.billingClient.acknowledgePurchase(AcknowledgePurchaseParams.newBuilder().setPurchaseToken(purchase.getPurchaseToken()).build(), (AcknowledgePurchaseResponseListener)new ProVersionActivityDone$$ExternalSyntheticLambda2());
            ...
            this.runOnUiThread((Runnable)new ProVersionActivityDone$8(this));
        }
        else {
            this.runOnUiThread((Runnable)new ProVersionActivityDone$9(this));
        }
    }
    ...
    private void initBtnHelp(final boolean b) {
        ...
        button.setOnClickListener((View$OnClickListener)new ProVersionActivityDone$4(this));
    }
    ...
    private void playVibration() {
        this.runOnUiThread((Runnable)new ProVersionActivityDone$15(this));
    }
    
    private void queryProducts() {
        final ArrayList productList = new ArrayList();
        productList.add(QueryProductDetailsParams$Product.newBuilder().setProductId(ProVersionActivityDone.PRODUCT_ID_FOREIVER).setProductType("inapp").build());
        this.billingClient.queryProductDetailsAsync(QueryProductDetailsParams.newBuilder().setProductList((List)productList).build(), (ProductDetailsResponseListener)new ProVersionActivityDone$6(this));
    }
    
    private void queryPurchases() {
        this.billingClient.queryPurchasesAsync(QueryPurchasesParams.newBuilder().setProductType("inapp").build(), (PurchasesResponseListener)new ProVersionActivityDone$$ExternalSyntheticLambda4(this));
    }
    
    private void querySubscribe() {
        final ArrayList productList = new ArrayList();
        final QueryProductDetailsParams$Product$Builder setProductId = QueryProductDetailsParams$Product.newBuilder().setProductId(ProVersionActivityDone.PRODUCT_ID_YEAR);
        final String s = "subs";
        productList.add(setProductId.setProductType(s).build());
        productList.add(QueryProductDetailsParams$Product.newBuilder().setProductId(ProVersionActivityDone.PRODUCT_ID_MONTH).setProductType(s).build());
        this.billingClient.queryProductDetailsAsync(QueryProductDetailsParams.newBuilder().setProductList((List)productList).build(), (ProductDetailsResponseListener)new ProVersionActivityDone$7(this));
    }
    
    private void queryUserPurchases() {
        this.billingClient.queryPurchasesAsync(QueryPurchasesParams.newBuilder().setProductType("inapp").build(), (PurchasesResponseListener)new ProVersionActivityDone$$ExternalSyntheticLambda1(this));
    }
    
    private void setupImg() {
        (this.recyclerView = (RecyclerView)this.findViewById(R$id.rv_img)).post((Runnable)new ProVersionActivityDone$11(this));
    }
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        try {
                                            final ProVersionActivityDone$3 onClickListener = new ProVersionActivityDone$3(this);
    ...
    private void startBillingConnection() {
        this.billingClient.startConnection((BillingClientStateListener)new ProVersionActivityDone$5(this));
    }
    ...
    private void thnks() {
        ((TextCustumFont)this.findViewById(R$id.tv_thanks)).setText((CharSequence)this.mResources.getString(R$string.thanks_hint));
    ...
    private void updateUI(final ProductDetails productDetails) {
        if (productDetails.getProductType().equals("inapp")) {
            ...
            if (ProVersionActivityDone.PRODUCT_ID_YEAR.equals(productId)) {
            ...
            else if (ProVersionActivityDone.PRODUCT_ID_MONTH.equals(productId)) {
    ...
    private void updateUILast(final ProductDetails productDetails) {
        ...
        if (equals) {
            ...
            if (ProVersionActivityDone.PRODUCT_ID_YEAR.equals(productDetails.getProductId())) {
    ...
    public void explode() {
        ...
        final PartyFactory shapes = new PartyFactory(new Emitter(3500L, TimeUnit.MILLISECONDS).max(300)).spread(360).shapes(ProVersionActivity$$ExternalSyntheticBackport0.m$1(loadDrawable));
    ...
    protected void onCreate(final Bundle bundle) {
        ...
        this.setContentView(R$layout.activity_pro_version_done);
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this.findViewById(R$id.main), (OnApplyWindowInsetsListener)new ProVersionActivityDone$$ExternalSyntheticLambda5());
        ...
        this.wakeLockAquire();
        ((TextView)this.findViewById(R$id.mtittle)).setText((CharSequence)this.mResources.getString(R$string.enjoy_all_premium_features_last));
        ...
        this.findViewById(R$id.btn_on_back).setOnClickListener((View$OnClickListener)new ProVersionActivityDone$2(this));
        this.setupImg();
        this.thnks();
    }
