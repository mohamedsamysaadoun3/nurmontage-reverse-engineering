...
public class EntityTrslTimeline
extends Entity {
    ...
    public EntityTrslTimeline(TranslationQuranEntity translationQuranEntity, float f, float f2, float f3, float f4, float f5) {
        ...
        super(f, f2, f4, f3);
        this.rect = translationQuranEntity;
        this.left = f6 = this.rect.left;
        this.right = f6 = this.rect.right;
        this.color = -67133;
        super(1);
        this.paintText = translationQuranEntity;
        Object object = Paint.Style.FILL;
        translationQuranEntity.setStyle((Paint.Style)object);
        translationQuranEntity = this.paintText;
        f = this.rect.height() * 0.27f;
        translationQuranEntity.setTextSize(f);
        translationQuranEntity = this.paintText;
        object = this.getQuranEntity().getPaintAya().getTypeface();
        translationQuranEntity.setTypeface((Typeface)object);
        this.paintText.setColor(-16441312);
        super();
        this.textBound = translationQuranEntity;
        translationQuranEntity = this.paintText;
        object = this.quranEntity.getTxt();
        int n = this.quranEntity.getTxt().length();
        Rect rect = this.textBound;
        translationQuranEntity.getTextBounds((String)object, 0, n, rect);
        f6 = this.rect.top;
        f = this.rect.height();
        f2 = 0.5f;
        f6 += (f *= f2);
        ...
        this.rectFLeft = translationQuranEntity;
        f = this.rectFLeft.width();
        super(0.0f, 0.0f, f, f3);
        this.rectFRight = translationQuranEntity;
