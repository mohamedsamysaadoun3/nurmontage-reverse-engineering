...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m3214constructorimpl = SegmentOrClosed.m3214constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m3214constructorimpl = SegmentOrClosed.m3214constructorimpl(segment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m3219isClosedimpl(m3214constructorimpl)) {
                Segment m3217getSegmentimpl = SegmentOrClosed.m3217getSegmentimpl(m3214constructorimpl);
                while (true) {
                    Segment segment4 = (Segment) this.tail;
                    if (segment4.getId() >= m3217getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!m3217getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (C0127xc40028dd.m6m(tail$FU, this, segment4, m3217getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m3217getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m3217getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m3217getSegmentimpl(m3214constructorimpl);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (andIncrement % i2);
        if (!C1658xa7a47114.m567m(semaphoreSegment.acquirers, i3, null, cont)) {
            symbol = SemaphoreKt.PERMIT;
            symbol2 = SemaphoreKt.TAKEN;
            if (C1658xa7a47114.m567m(semaphoreSegment.acquirers, i3, symbol, symbol2)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (C0127xc40028dd.m6m(head$FU, this, segment4, m3217getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m3217getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !C1658xa7a47114.m567m(semaphoreSegment.acquirers, i4, symbol3, symbol4);
        }
