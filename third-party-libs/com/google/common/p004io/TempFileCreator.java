...
import com.google.common.p004io.TempFileCreator;
import java.io.File;
...
abstract class TempFileCreator {
    ...
    static final class JavaNioCreator extends TempFileCreator {
        ...
        @Override // com.google.common.p004io.TempFileCreator
        File createTempDir() {
            try {
        ...
        public static PermissionSupplier userPermissions() {
            try {
                final ImmutableList m394of = ImmutableList.m394of(AclEntry.newBuilder().setType(AclEntryType.ALLOW).setPrincipal(FileSystems.getDefault().getUserPrincipalLookupService().lookupPrincipalByName(getUsername())).setPermissions(EnumSet.allOf(AclEntryPermission.class)).setFlags(AclEntryFlag.DIRECTORY_INHERIT, AclEntryFlag.FILE_INHERIT).build());
