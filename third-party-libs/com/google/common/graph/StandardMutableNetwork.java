...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N nodeU, N nodeV, E edge) {
        ...
        if (containsEdge(edge)) {
            ...
            EndpointPair m514of = EndpointPair.m514of(this, nodeU, nodeV);
            Preconditions.checkArgument(incidentNodes.equals(m514of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", edge, incidentNodes, m514of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m511of();
            }
            return DirectedNetworkConnections.m512of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m519of();
        }
        return UndirectedNetworkConnections.m520of();
    }
}
