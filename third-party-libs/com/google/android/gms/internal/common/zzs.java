...
public final class zzs {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            zza = zzy.zza("%s (%s) must not be negative", StreamInformation.KEY_INDEX, Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException("negative size: " + i2);
            }
            zza = zzy.zza("%s (%s) must be less than size (%s)", StreamInformation.KEY_INDEX, Integer.valueOf(i), Integer.valueOf(i2));
        }
