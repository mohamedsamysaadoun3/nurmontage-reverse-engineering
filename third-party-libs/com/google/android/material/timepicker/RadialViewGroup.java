...
class RadialViewGroup extends ConstraintLayout {
    ...
    public RadialViewGroup(Context context, AttributeSet attributeSet, int i) {
        ...
        LayoutInflater.from(context).inflate(C0904R.layout.material_radial_view_group, this);
        ViewCompat.setBackground(this, createBackground());
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0904R.styleable.RadialViewGroup, i, 0);
        this.radius = obtainStyledAttributes.getDimensionPixelSize(C0904R.styleable.RadialViewGroup_materialCircleRadius, 0);
    ...
    protected void updateLayoutParams() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childAt.getId() != C0904R.id.circle_center && !shouldSkipView(childAt)) {
                int i2 = (Integer) childAt.getTag(C0904R.id.material_clock_level);
                if (i2 == null) {
                    i2 = 1;
                }
    ...
    private void addConstraints(List<View> list, ConstraintSet constraintSet, int i) {
        ...
        float f = 0.0f;
        while (it.hasNext()) {
            constraintSet.constrainCircle(it.next().getId(), C0904R.id.circle_center, i, f);
            f += 360.0f / list.size();
        }
    }
