...
public abstract class MediaCodecRenderer extends BaseRenderer {
    private static final byte[] ADAPTATION_WORKAROUND_BUFFER = {0, 0, 1, 103, 66, -64, Ascii.f264VT, -38, 37, -112, 0, 0, 1, 104, -50, Ascii.f260SI, 19, 32, 0, 0, 1, 101, -120, -124, Ascii.f251CR, -50, 113, Ascii.CAN, -96, 0, 47, -65, Ascii.f254FS, 49, -61, 39, 93, 120};
    ...
    public MediaCodecRenderer(int i, MediaCodecAdapter.Factory factory, MediaCodecSelector mediaCodecSelector, boolean z, float f) {
        ...
        this.renderTimeLimitMs = C0366C.TIME_UNSET;
        ...
        this.inputIndex = -1;
        this.outputIndex = -1;
        this.codecHotswapDeadlineMs = C0366C.TIME_UNSET;
        this.largestQueuedPresentationTimeUs = C0366C.TIME_UNSET;
        this.lastBufferInStreamPresentationTimeUs = C0366C.TIME_UNSET;
        this.lastProcessedOutputBufferTimeUs = C0366C.TIME_UNSET;
        this.lastOutputBufferProcessedRealtimeMs = C0366C.TIME_UNSET;
        this.codecDrainState = 0;
        this.codecDrainAction = 0;
        this.decoderCounters = new DecoderCounters();
        this.skippedFlushLastOutputBufferPresentationTimeUs = C0366C.TIME_UNSET;
        this.largestQueuedPresentationTimeOfExpectedOutputBufferUs = C0366C.TIME_UNSET;
    }
    ...
    */
    protected void onStreamChanged(Format[] formatArr, long j, long j2, MediaSource.MediaPeriodId mediaPeriodId) throws ExoPlaybackException {
        if (this.outputStreamInfo.streamOffsetUs == C0366C.TIME_UNSET) {
            setOutputStreamInfo(new OutputStreamInfo(C0366C.TIME_UNSET, j, j2));
    ...
    protected boolean shouldReleaseCodecInsteadOfFlushing() {
        ...
        try {
            ...
            Log.m62w(TAG, "Failed to update the DRM session, releasing the codec instead.", e);
            return true;
        }
    }

    private void onSkippedFlushCodec() {
        if (this.largestQueuedPresentationTimeOfExpectedOutputBufferUs != C0366C.TIME_UNSET) {
    ...
    private void maybeInitCodecWithFallback(MediaCrypto mediaCrypto, boolean z) throws DecoderInitializationException, ExoPlaybackException {
        ...
        while (this.codec == null) {
            ...
            try {
                ...
                Log.m62w(TAG, "Failed to initialize decoder: " + mediaCodecInfo, e2);
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean z) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (decoderInfos.isEmpty() && z) {
            ...
            if (!decoderInfos.isEmpty()) {
                Log.m61w(TAG, "Drm session requires secure decoder for " + format.sampleMimeType + ", but no secure decoder available. Trying to proceed with " + decoderInfos + ".");
            }
        }
        return decoderInfos;
    }
    ...
    private void initCodec(MediaCodecInfo mediaCodecInfo, MediaCrypto mediaCrypto) throws Exception {
        ...
        try {
            ...
            if (!mediaCodecInfo.isFormatSupported(format)) {
                Log.m61w(TAG, Util.formatInvariant("Format exceeds selected codec's capabilities [%s, %s]", Format.toLogString(format), str));
            }
    ...
    private boolean shouldContinueRendering(long j) {
        return this.renderTimeLimitMs == C0366C.TIME_UNSET || getClock().elapsedRealtime() - j < this.renderTimeLimitMs;
    }
    ...
    */
    ...
    public boolean isReady() {
        return this.inputFormat != null && (isSourceReady() || hasOutputBuffer() || (this.codecHotswapDeadlineMs != C0366C.TIME_UNSET && getClock().elapsedRealtime() < this.codecHotswapDeadlineMs));
    }
    ...
    private boolean drainOutputBuffer(long j, long j2) throws ExoPlaybackException {
        ...
        if (!hasOutputBuffer()) {
            ...
            if (dequeueOutputBufferIndex < 0) {
                ...
                if (j3 != C0366C.TIME_UNSET && j3 + 100 < getClock().currentTimeMillis()) {
                    processEndOfStream();
                }
                return false;
            }
        ...
        this.isLastOutputBuffer = j4 != C0366C.TIME_UNSET && j4 <= this.outputBufferInfo.presentationTimeUs;
        if (this.skippedFlushAndWaitingForEarlierFrame) {
            if (this.skippedFlushLastOutputBufferPresentationTimeUs != C0366C.TIME_UNSET && this.outputBufferInfo.presentationTimeUs <= this.skippedFlushLastOutputBufferPresentationTimeUs) {
                this.skippedFlushAndWaitingForEarlierFrame = false;
                this.skippedFlushLastOutputBufferPresentationTimeUs = C0366C.TIME_UNSET;
            } else {
    ...
    private void setOutputStreamInfo(OutputStreamInfo outputStreamInfo) {
        ...
        if (outputStreamInfo.streamOffsetUs != C0366C.TIME_UNSET) {
    ...
    private boolean drmNeedsCodecReinitialization(MediaCodecInfo mediaCodecInfo, Format format, DrmSession drmSession, DrmSession drmSession2) throws ExoPlaybackException {
        ...
        if (drmSession2 != null && drmSession != null && (cryptoConfig = drmSession2.getCryptoConfig()) != null && (cryptoConfig2 = drmSession.getCryptoConfig()) != null && cryptoConfig.getClass().equals(cryptoConfig2.getClass())) {
            ...
            if (drmSession2.getSchemeUuid().equals(drmSession.getSchemeUuid()) && !C0366C.PLAYREADY_UUID.equals(drmSession.getSchemeUuid()) && !C0366C.PLAYREADY_UUID.equals(drmSession2.getSchemeUuid())) {
    ...
    private static final class OutputStreamInfo {
        public static final OutputStreamInfo UNSET = new OutputStreamInfo(C0366C.TIME_UNSET, C0366C.TIME_UNSET, C0366C.TIME_UNSET);
