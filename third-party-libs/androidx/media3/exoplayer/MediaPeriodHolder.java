...
final class MediaPeriodHolder {
    ...
    public MediaPeriodHolder(RendererCapabilities[] rendererCapabilitiesArr, long j, TrackSelector trackSelector, Allocator allocator, MediaSourceList mediaSourceList, MediaPeriodInfo mediaPeriodInfo, TrackSelectorResult trackSelectorResult, long j2) {
        ...
        this.uid = mediaPeriodInfo.f104id.periodUid;
        this.info = mediaPeriodInfo;
        this.targetPreloadBufferDurationUs = j2;
        this.trackSelectorResult = trackSelectorResult;
        this.sampleStreams = new SampleStream[rendererCapabilitiesArr.length];
        this.mayRetainStreamFlags = new boolean[rendererCapabilitiesArr.length];
        this.mediaPeriod = createMediaPeriod(mediaPeriodInfo.f104id, mediaSourceList, allocator, mediaPeriodInfo.startPositionUs, mediaPeriodInfo.endPositionUs, mediaPeriodInfo.isPrecededByTransitionFromSameStream);
    }
    ...
    public void handlePrepared(float f, Timeline timeline, boolean z) throws ExoPlaybackException {
        ...
        if (this.info.durationUs != C0366C.TIME_UNSET && j >= this.info.durationUs) {
    ...
    public void updateClipping() {
        if (this.mediaPeriod instanceof ClippingMediaPeriod) {
            ((ClippingMediaPeriod) this.mediaPeriod).updateClipping(0L, this.info.endPositionUs == C0366C.TIME_UNSET ? Long.MIN_VALUE : this.info.endPositionUs);
        }
    }
    ...
    private static MediaPeriod createMediaPeriod(MediaSource.MediaPeriodId mediaPeriodId, MediaSourceList mediaSourceList, Allocator allocator, long j, long j2, boolean z) {
        ...
        return j2 != C0366C.TIME_UNSET ? new ClippingMediaPeriod(createPeriod, !z, 0L, j2) : createPeriod;
    }
    ...
    private static void releaseMediaPeriod(MediaSourceList mediaSourceList, MediaPeriod mediaPeriod) {
        try {
            ...
            Log.m58e(TAG, "Period release failed.", e);
        }
    }

    public boolean canBeUsedForMediaPeriodInfo(MediaPeriodInfo mediaPeriodInfo) {
        return MediaPeriodQueue.areDurationsCompatible(this.info.durationUs, mediaPeriodInfo.durationUs) && this.info.startPositionUs == mediaPeriodInfo.startPositionUs && this.info.f104id.equals(mediaPeriodInfo.f104id);
    }
