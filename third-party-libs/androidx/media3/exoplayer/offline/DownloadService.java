...
public abstract class DownloadService extends Service {
        ...
        switch (str) {
            case "androidx.media3.exoplayer.downloadService.action.SET_STOP_REASON":
                if (!((Intent) Assertions.checkNotNull(intent)).hasExtra(KEY_STOP_REASON)) {
                    Log.m57e(TAG, "Ignored SET_STOP_REASON: Missing stop_reason extra");
                    break;
                } else {
            ...
            case "androidx.media3.exoplayer.downloadService.action.REMOVE_DOWNLOAD":
                if (str2 == null) {
                    Log.m57e(TAG, "Ignored REMOVE_DOWNLOAD: Missing content_id extra");
                    break;
                } else {
            ...
            case "androidx.media3.exoplayer.downloadService.action.ADD_DOWNLOAD":
                ...
                if (downloadRequest == null) {
                    Log.m57e(TAG, "Ignored ADD_DOWNLOAD: Missing download_request extra");
                    break;
                } else {
            ...
            case "androidx.media3.exoplayer.downloadService.action.SET_REQUIREMENTS":
                ...
                if (requirements == null) {
                    Log.m57e(TAG, "Ignored SET_REQUIREMENTS: Missing requirements extra");
                    break;
                } else {
            ...
            case "androidx.media3.exoplayer.downloadService.action.PAUSE_DOWNLOADS":
                ...
                break;
            default:
                Log.m57e(TAG, "Ignored unrecognized action: " + str);
                break;
        }
    ...
    static final class DownloadManagerHelper implements DownloadManager.Listener {
        ...
        public void attachService(final DownloadService downloadService) {
            ...
            if (this.downloadManager.isInitialized()) {
                Util.createHandlerForCurrentOrMainLooper().postAtFrontOfQueue(new Runnable() { // from class: androidx.media3.exoplayer.offline.DownloadService$DownloadManagerHelper$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        DownloadService.DownloadManagerHelper.this.m143xee6ee963(downloadService);
                    }
                });
            }
        }

        /* renamed from: lambda$attachService$0$androidx-media3-exoplayer-offline-DownloadService$DownloadManagerHelper */
        /* synthetic */ void m143xee6ee963(DownloadService downloadService) {
        ...
        public boolean updateScheduler() {
            ...
            Log.m61w(DownloadService.TAG, "Failed to schedule restart");
            cancelScheduler();
            return false;
        }
        ...
        public void onDownloadChanged(DownloadManager downloadManager, Download download, Exception exc) {
            ...
            if (serviceMayNeedRestart() && DownloadService.needsStartedService(download.state)) {
                Log.m61w(DownloadService.TAG, "DownloadService wasn't running. Restarting.");
                restartService();
            }
        }
        ...
        private void restartService() {
            if (this.foregroundAllowed) {
                try {
                    ...
                    Log.m61w(DownloadService.TAG, "Failed to restart (foreground launch restriction)");
                    return;
                }
            }
            try {
                ...
                Log.m61w(DownloadService.TAG, "Failed to restart (process is idle)");
            }
        }
    }
}
