...
final class MergingCuesResolver implements CuesResolver {
    ...
    public boolean addCues(CuesWithTiming cuesWithTiming, long j) {
        ...
        Assertions.checkArgument(cuesWithTiming.durationUs != C0366C.TIME_UNSET);
    ...
    public long getNextCueChangeTimeUs(long j) {
        ...
        while (true) {
            ...
            long j4 = this.cuesWithTimingList.get(i).endTimeUs;
            if (j < j3) {
                j2 = j2 == C0366C.TIME_UNSET ? j3 : Math.min(j2, j3);
            } else {
                if (j < j4) {
                    j2 = j2 == C0366C.TIME_UNSET ? j4 : Math.min(j2, j4);
                }
                i++;
            }
        }
