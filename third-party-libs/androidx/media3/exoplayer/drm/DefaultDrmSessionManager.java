...
public class DefaultDrmSessionManager implements DrmSessionManager {
    ...
    public static final class Builder {
        ...
        private UUID uuid = C0366C.WIDEVINE_UUID;
    ...
    public final void prepare() {
        ...
        } else if (this.sessionKeepaliveMs != C0366C.TIME_UNSET) {
    ...
    public final void release() {
        ...
        if (this.sessionKeepaliveMs != C0366C.TIME_UNSET) {
    ...
    public DrmSession acquireSession(Looper looper, DrmSessionEventListener.EventDispatcher eventDispatcher, Format format, boolean z) {
        ...
        if (this.offlineLicenseKeySetId == null) {
            ...
            if (list.isEmpty()) {
                ...
                Log.m58e(TAG, "DRM error", missingSchemeDataException);
    ...
    private DrmSession maybeAcquirePlaceholderSession(int i, boolean z) {
        ...
        if (defaultDrmSession == null) {
            DefaultDrmSession createAndAcquireSessionWithRetry = createAndAcquireSessionWithRetry(ImmutableList.m393of(), true, null, z);
    ...
    private boolean canAcquireSession(DrmInitData drmInitData) {
        ...
        if (getSchemeDatas(drmInitData, this.uuid, true).isEmpty()) {
            if (drmInitData.schemeDataCount != 1 || !drmInitData.get(0).matches(C0366C.COMMON_PSSH_UUID)) {
                return false;
            }
            Log.m61w(TAG, "DrmInitData only contains common PSSH SchemeData. Assuming support for: " + this.uuid);
        }
        String str = drmInitData.schemeType;
        if (str == null || C0366C.CENC_TYPE_cenc.equals(str)) {
            return true;
        }
        return C0366C.CENC_TYPE_cbcs.equals(str) ? Build.VERSION.SDK_INT >= 25 : (C0366C.CENC_TYPE_cbc1.equals(str) || C0366C.CENC_TYPE_cens.equals(str)) ? false : true;
    }
    ...
    private void verifyPlaybackThread(boolean z) {
        if (z && this.playbackLooper == null) {
            Log.m62w(TAG, "DefaultDrmSessionManager accessed before setPlayer(), possibly on the wrong thread.", new IllegalStateException());
        } else if (Thread.currentThread() != ((Looper) Assertions.checkNotNull(this.playbackLooper)).getThread()) {
            Log.m62w(TAG, "DefaultDrmSessionManager accessed on the wrong thread.\nCurrent thread: " + Thread.currentThread().getName() + "\nExpected thread: " + this.playbackLooper.getThread().getName(), new IllegalStateException());
        }
    }
    ...
    private static List<DrmInitData.SchemeData> getSchemeDatas(DrmInitData drmInitData, UUID uuid, boolean z) {
        ...
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if ((schemeData.matches(uuid) || (C0366C.CLEARKEY_UUID.equals(uuid) && schemeData.matches(C0366C.COMMON_PSSH_UUID))) && (schemeData.data != null || z)) {
    ...
    class ReferenceCountListenerImpl implements DefaultDrmSession.ReferenceCountListener {
        ...
        public void onReferenceCountIncremented(DefaultDrmSession defaultDrmSession, int i) {
            if (DefaultDrmSessionManager.this.sessionKeepaliveMs != C0366C.TIME_UNSET) {
        ...
        public void onReferenceCountDecremented(final DefaultDrmSession defaultDrmSession, int i) {
            if (i == 1 && DefaultDrmSessionManager.this.prepareCallsCount > 0 && DefaultDrmSessionManager.this.sessionKeepaliveMs != C0366C.TIME_UNSET) {
    ...
    class PreacquiredSessionReference implements DrmSessionManager.DrmSessionReference {
        ...
        public void acquire(final Format format) {
            ((Handler) Assertions.checkNotNull(DefaultDrmSessionManager.this.playbackHandler)).post(new Runnable() { // from class: androidx.media3.exoplayer.drm.DefaultDrmSessionManager$PreacquiredSessionReference$$ExternalSyntheticLambda0
                @Override // java.lang.Runnable
                public final void run() {
                    DefaultDrmSessionManager.PreacquiredSessionReference.this.m122x937f548e(format);
                }
            });
        }

        /* renamed from: lambda$acquire$0$androidx-media3-exoplayer-drm-DefaultDrmSessionManager$PreacquiredSessionReference */
        /* synthetic */ void m122x937f548e(Format format) {
        ...
        public void release() {
            Util.postOrRun((Handler) Assertions.checkNotNull(DefaultDrmSessionManager.this.playbackHandler), new Runnable() { // from class: androidx.media3.exoplayer.drm.DefaultDrmSessionManager$PreacquiredSessionReference$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
                    DefaultDrmSessionManager.PreacquiredSessionReference.this.m123xce10c5c();
                }
            });
        }

        /* renamed from: lambda$release$1$androidx-media3-exoplayer-drm-DefaultDrmSessionManager$PreacquiredSessionReference */
        /* synthetic */ void m123xce10c5c() {
            if (this.isReleased) {
                return;
            }
