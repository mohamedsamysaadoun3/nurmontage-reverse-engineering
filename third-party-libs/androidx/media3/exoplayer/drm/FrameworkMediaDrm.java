...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    ...
    static /* synthetic */ ExoMediaDrm lambda$static$0(UUID uuid) {
        try {
            ...
            Log.m57e(TAG, "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
    ...
    private FrameworkMediaDrm(UUID uuid) throws UnsupportedSchemeException {
        ...
        Assertions.checkArgument(!C0366C.COMMON_PSSH_UUID.equals(uuid), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid;
        MediaDrm mediaDrm = new MediaDrm(adjustUuid(uuid));
        this.mediaDrm = mediaDrm;
        this.referenceCount = 1;
        if (C0366C.WIDEVINE_UUID.equals(uuid) && needsForceWidevineL3Workaround()) {
    ...
    public void setOnEventListener(final ExoMediaDrm.OnEventListener onEventListener) {
        this.mediaDrm.setOnEventListener(onEventListener == null ? null : new MediaDrm.OnEventListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda2
            ...
            public final void onEvent(MediaDrm mediaDrm, byte[] bArr, int i, int i2, byte[] bArr2) {
                FrameworkMediaDrm.this.m130x5e84e274(onEventListener, mediaDrm, bArr, i, i2, bArr2);
            }
        });
    }

    /* renamed from: lambda$setOnEventListener$1$androidx-media3-exoplayer-drm-FrameworkMediaDrm */
    /* synthetic */ void m130x5e84e274(ExoMediaDrm.OnEventListener onEventListener, MediaDrm mediaDrm, byte[] bArr, int i, int i2, byte[] bArr2) {
    ...
    public void setOnKeyStatusChangeListener(final ExoMediaDrm.OnKeyStatusChangeListener onKeyStatusChangeListener) {
        this.mediaDrm.setOnKeyStatusChangeListener(onKeyStatusChangeListener == null ? null : new MediaDrm.OnKeyStatusChangeListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda3
            ...
            public final void onKeyStatusChange(MediaDrm mediaDrm, byte[] bArr, List list, boolean z) {
                FrameworkMediaDrm.this.m132xc78bb65c(onKeyStatusChangeListener, mediaDrm, bArr, list, z);
            }
        }, (Handler) null);
    }

    /* renamed from: lambda$setOnKeyStatusChangeListener$2$androidx-media3-exoplayer-drm-FrameworkMediaDrm */
    /* synthetic */ void m132xc78bb65c(ExoMediaDrm.OnKeyStatusChangeListener onKeyStatusChangeListener, MediaDrm mediaDrm, byte[] bArr, List list, boolean z) {
    ...
    public void setOnExpirationUpdateListener(final ExoMediaDrm.OnExpirationUpdateListener onExpirationUpdateListener) {
        this.mediaDrm.setOnExpirationUpdateListener(onExpirationUpdateListener == null ? null : new MediaDrm.OnExpirationUpdateListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda1
            ...
            public final void onExpirationUpdate(MediaDrm mediaDrm, byte[] bArr, long j) {
                FrameworkMediaDrm.this.m131x3bcdcffc(onExpirationUpdateListener, mediaDrm, bArr, j);
            }
        }, (Handler) null);
    }

    /* renamed from: lambda$setOnExpirationUpdateListener$3$androidx-media3-exoplayer-drm-FrameworkMediaDrm */
    /* synthetic */ void m131x3bcdcffc(ExoMediaDrm.OnExpirationUpdateListener onExpirationUpdateListener, MediaDrm mediaDrm, byte[] bArr, long j) {
    ...
    public void setPlayerIdForSession(byte[] bArr, PlayerId playerId) {
        if (Build.VERSION.SDK_INT >= 31) {
            try {
                ...
                Log.m61w(TAG, "setLogSessionId failed.");
            }
        }
    }
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws NotProvisionedException, DeniedByServerException {
        if (C0366C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public boolean requiresSecureDecoder(byte[] bArr, String str) {
        ...
        try {
            ...
            z = !this.uuid.equals(C0366C.CLEARKEY_UUID);
    ...
    private boolean isMediaDrmRequiresSecureDecoderImplemented() {
        if (this.uuid.equals(C0366C.WIDEVINE_UUID)) {
            String propertyString = getPropertyString("version");
    ...
    private static UUID adjustUuid(UUID uuid) {
        return cdmRequiresCommonPsshUuid(uuid) ? C0366C.COMMON_PSSH_UUID : uuid;
    }

    private static byte[] adjustRequestInitData(UUID uuid, byte[] bArr) {
        byte[] parseSchemeSpecificData;
        PsshAtomUtil.PsshAtom parsePsshAtom;
        if (C0366C.PLAYREADY_UUID.equals(uuid)) {
            ...
            bArr = PsshAtomUtil.buildPsshAtom(C0366C.PLAYREADY_UUID, addLaUrlAttributeIfMissing(bArr));
        }
        if (cdmRequiresCommonPsshUuid(uuid) && (parsePsshAtom = PsshAtomUtil.parsePsshAtom(bArr)) != null) {
            bArr = PsshAtomUtil.buildPsshAtom(C0366C.COMMON_PSSH_UUID, parsePsshAtom.keyIds, parsePsshAtom.schemeData);
        }
        return (C0366C.PLAYREADY_UUID.equals(uuid) && "Amazon".equals(Build.MANUFACTURER) && ("AFTB".equals(Build.MODEL) || "AFTS".equals(Build.MODEL) || "AFTM".equals(Build.MODEL) || "AFTT".equals(Build.MODEL)) && (parseSchemeSpecificData = PsshAtomUtil.parseSchemeSpecificData(bArr, uuid)) != null) ? parseSchemeSpecificData : bArr;
    }

    private static String adjustRequestMimeType(UUID uuid, String str) {
        return (Build.VERSION.SDK_INT < 26 && C0366C.CLEARKEY_UUID.equals(uuid) && (MimeTypes.VIDEO_MP4.equals(str) || MimeTypes.AUDIO_MP4.equals(str))) ? "cenc" : str;
    }

    private static byte[] adjustRequestData(UUID uuid, byte[] bArr) {
        return C0366C.CLEARKEY_UUID.equals(uuid) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }

    private static boolean cdmRequiresCommonPsshUuid(UUID uuid) {
        return Build.VERSION.SDK_INT < 27 && Objects.equals(uuid, C0366C.CLEARKEY_UUID);
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (readLittleEndianShort != 1 || readLittleEndianShort2 != 1) {
            Log.m59i(TAG, "Unexpected record count or type. Skipping LA_URL workaround.");
            return bArr;
        }
        ...
        if (indexOf == -1) {
            Log.m61w(TAG, "Could not find the </DATA> tag. Skipping LA_URL workaround.");
        }
