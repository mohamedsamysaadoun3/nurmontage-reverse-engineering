...
public final class HttpMediaDrmCallback implements MediaDrmCallback {
    ...
    public byte[] executeKeyRequest(UUID uuid, ExoMediaDrm.KeyRequest keyRequest) throws MediaDrmCallbackException {
        ...
        if (TextUtils.isEmpty(licenseServerUrl)) {
            throw new MediaDrmCallbackException(new DataSpec.Builder().setUri(Uri.EMPTY).build(), Uri.EMPTY, ImmutableMap.m412of(), 0L, new IllegalStateException("No license URL"));
        }
        HashMap hashMap = new HashMap();
        if (C0366C.PLAYREADY_UUID.equals(uuid)) {
            str = "text/xml";
        } else {
            str = C0366C.CLEARKEY_UUID.equals(uuid) ? "application/json" : "application/octet-stream";
        }
