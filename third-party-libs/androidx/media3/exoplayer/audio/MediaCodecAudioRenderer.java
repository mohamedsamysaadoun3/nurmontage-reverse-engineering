...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    private static List<MediaCodecInfo> getDecoderInfos(MediaCodecSelector mediaCodecSelector, Format format, boolean z, AudioSink audioSink) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (audioSink.supportsFormat(format) && (decryptOnlyDecoderInfo = MediaCodecUtil.getDecryptOnlyDecoderInfo()) != null) {
            return ImmutableList.m394of(decryptOnlyDecoderInfo);
        }
    ...
    protected long getDurationToProgressUs(long j, long j2, boolean z) {
        boolean z2 = this.nextBufferToWritePresentationTimeUs != C0366C.TIME_UNSET;
        if (!this.isStarted) {
            if (z2 || super.isEnded()) {
                return 1000000L;
            }
        ...
        if (!z2 || audioTrackBufferSizeUs == C0366C.TIME_UNSET) {
            return Renderer.DEFAULT_DURATION_TO_PROGRESS_US;
        }
    ...
    protected void onCodecError(Exception exc) {
        Log.m58e(TAG, "Audio codec error", exc);
    ...
    protected void renderToEndOfStream() throws ExoPlaybackException {
        try {
            ...
            if (getLastBufferInStreamPresentationTimeUs() != C0366C.TIME_UNSET) {
    ...
    protected void handleInputBufferSupplementalData(DecoderInputBuffer decoderInputBuffer) {
        ...
        if (byteBuffer.remaining() == 8) {
            this.audioSink.setOffloadDelayPadding(i, (int) ((byteBuffer.order(ByteOrder.LITTLE_ENDIAN).getLong() * 48000) / C0366C.NANOS_PER_SECOND));
        }
    }
    ...
    private final class AudioSinkListener implements AudioSink.Listener {
        ...
        public void onAudioSinkError(Exception exc) {
            Log.m58e(MediaCodecAudioRenderer.TAG, "Audio sink error", exc);
