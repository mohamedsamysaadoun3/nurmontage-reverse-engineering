...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk>, Loader.ReleaseCallback {
    ...
    public void seekToUs(long j) {
        ...
        for (int i2 = 0; i2 < this.mediaChunks.size(); i2++) {
            ...
            if (j2 == j && baseMediaChunk.clippedStartTimeUs == C0366C.TIME_UNSET) {
                break;
            } else {
                if (j2 > j) {
                    break;
    ...
    */
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j, long j2, IOException iOException, int i) {
        ...
        if (this.chunkSource.onChunkLoadError(chunk, z, loadErrorInfo, this.loadErrorHandlingPolicy)) {
            ...
            Log.m61w(TAG, "Ignoring attempt to cancel non-cancelable load.");
        }
    ...
    public void discardUpstreamSamplesForClippedDuration(long j) {
        ...
        if (isPendingReset() || j == C0366C.TIME_UNSET || this.mediaChunks.isEmpty()) {
            return;
        }
        BaseMediaChunk lastMediaChunk = getLastMediaChunk();
        if (lastMediaChunk.clippedEndTimeUs != C0366C.TIME_UNSET) {
