...
final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput, Loader.Callback<ExtractingLoadable>, Loader.ReleaseCallback, SampleQueue.UpstreamFormatChangedListener {
    ...
    public ProgressiveMediaPeriod(Uri uri, DataSource dataSource, ProgressiveMediaExtractor progressiveMediaExtractor, DrmSessionManager drmSessionManager, DrmSessionEventListener.EventDispatcher eventDispatcher, LoadErrorHandlingPolicy loadErrorHandlingPolicy, MediaSourceEventListener.EventDispatcher eventDispatcher2, Listener listener, Allocator allocator, String str, int i, int i2, Format format, long j, ReleasableExecutor releasableExecutor) {
        ...
        this.onContinueLoadingRequestedRunnable = new Runnable() { // from class: androidx.media3.exoplayer.source.ProgressiveMediaPeriod$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
                ProgressiveMediaPeriod.this.m155x97cae34d();
            }
        };
        this.handler = Util.createHandlerForCurrentLooper();
        this.sampleQueueTrackIds = new TrackId[0];
        this.sampleQueues = new SampleQueue[0];
        this.pendingResetPositionUs = C0366C.TIME_UNSET;
        this.dataType = 1;
    }

    /* renamed from: lambda$new$0$androidx-media3-exoplayer-source-ProgressiveMediaPeriod */
    /* synthetic */ void m155x97cae34d() {
        if (this.released) {
            return;
        }
    ...
    public void prepare(MediaPeriod.Callback callback, long j) {
        ...
        if (this.singleTrackFormat != null) {
            ...
            m157x33ac0ff2(new IndexSeekMap(new long[]{0}, new long[]{0}, C0366C.TIME_UNSET));
            endTracks();
    ...
    public void onLoadCompleted(ExtractingLoadable extractingLoadable, long j, long j2) {
        if (this.durationUs == C0366C.TIME_UNSET && this.seekMap != null) {
    ...
    public Loader.LoadErrorAction onLoadError(ExtractingLoadable extractingLoadable, long j, long j2, IOException iOException, int i) {
        ...
        if (retryDelayMsFor == C0366C.TIME_UNSET) {
    ...
    public void seekMap(final SeekMap seekMap) {
        this.handler.post(new Runnable() { // from class: androidx.media3.exoplayer.source.ProgressiveMediaPeriod$$ExternalSyntheticLambda3
            @Override // java.lang.Runnable
            public final void run() {
                ProgressiveMediaPeriod.this.m157x33ac0ff2(seekMap);
            }
        });
    }
    ...
    public void onLengthKnown() {
        this.handler.post(new Runnable() { // from class: androidx.media3.exoplayer.source.ProgressiveMediaPeriod$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                ProgressiveMediaPeriod.this.m156xa2bd730d();
            }
        });
    }

    /* renamed from: lambda$onLengthKnown$2$androidx-media3-exoplayer-source-ProgressiveMediaPeriod */
    /* synthetic */ void m156xa2bd730d() {
    ...
    private TrackOutput prepareTrackOutput(TrackId trackId) {
        ...
        if (this.sampleQueuesBuilt) {
            Log.m61w(TAG, "Extractor added new track (id=" + trackId.f110id + ") after finishing tracks.");
    ...
    public void m157x33ac0ff2(SeekMap seekMap) {
        this.seekMap = this.icyHeaders == null ? seekMap : new SeekMap.Unseekable(C0366C.TIME_UNSET);
        this.durationUs = seekMap.getDurationUs();
        boolean z = !this.isLengthKnown && seekMap.getDurationUs() == C0366C.TIME_UNSET;
        this.isLive = z;
        this.dataType = z ? 7 : 1;
        if (this.prepared) {
    ...
    public void maybeFinishPrepare() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            this.isSingleSample = this.singleSampleDurationUs != C0366C.TIME_UNSET && length == 1 && MimeTypes.isImage(str);
        ...
        if (this.isSingleSample && this.durationUs == C0366C.TIME_UNSET) {
    ...
    private void startLoading() {
        ...
        if (this.prepared) {
            ...
            if (j != C0366C.TIME_UNSET && this.pendingResetPositionUs > j) {
                this.loadingFinished = true;
                this.pendingResetPositionUs = C0366C.TIME_UNSET;
                return;
            }
    ...
    private boolean configureRetry(ExtractingLoadable extractingLoadable, int i) {
        SeekMap seekMap;
        if (this.isLengthKnown || ((seekMap = this.seekMap) != null && seekMap.getDurationUs() != C0366C.TIME_UNSET)) {
    ...
    private boolean isPendingReset() {
        return this.pendingResetPositionUs != C0366C.TIME_UNSET;
    }
    ...
    private static final class TrackId {
        ...
        public boolean equals(Object obj) {
            ...
            return this.f110id == trackId.f110id && this.isIcyTrack == trackId.isIcyTrack;
        }

        public int hashCode() {
            return (this.f110id * 31) + (this.isIcyTrack ? 1 : 0);
        }
    }
