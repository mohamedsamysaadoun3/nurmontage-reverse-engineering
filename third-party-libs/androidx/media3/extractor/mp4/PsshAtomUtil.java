...
public final class PsshAtomUtil {
    ...
    public static byte[] parseSchemeSpecificData(byte[] bArr, UUID uuid) {
        ...
        if (!uuid.equals(parsePsshAtom.uuid)) {
            Log.m61w(TAG, "UUID mismatch. Expected: " + uuid + ", got: " + parsePsshAtom.uuid + ".");
            return null;
        }
    ...
    public static PsshAtom parsePsshAtom(byte[] bArr) {
        ...
        if (readInt != bytesLeft) {
            Log.m61w(TAG, "Advertised atom size (" + readInt + ") does not match buffer size: " + bytesLeft);
            return null;
        }
        int readInt2 = parsableByteArray.readInt();
        if (readInt2 != 1886614376) {
            Log.m61w(TAG, "Atom type is not pssh: " + readInt2);
            return null;
        }
        int parseFullBoxVersion = BoxParser.parseFullBoxVersion(parsableByteArray.readInt());
        if (parseFullBoxVersion > 1) {
            Log.m61w(TAG, "Unsupported pssh version: " + parseFullBoxVersion);
            return null;
        }
        ...
        if (readUnsignedIntToInt2 != bytesLeft2) {
            Log.m61w(TAG, "Atom data size (" + readUnsignedIntToInt2 + ") does not match the bytes left: " + bytesLeft2);
            return null;
        }
