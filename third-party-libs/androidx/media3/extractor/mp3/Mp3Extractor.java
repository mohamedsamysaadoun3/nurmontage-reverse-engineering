...
public final class Mp3Extractor implements Extractor {
    ...
    public Mp3Extractor(int i, long j) {
        ...
        this.basisTimeUs = C0366C.TIME_UNSET;
    ...
    private int readSample(ExtractorInput extractorInput) throws IOException {
        if (this.sampleBytesRemaining == 0) {
            ...
            if (this.basisTimeUs == C0366C.TIME_UNSET) {
                this.basisTimeUs = this.seeker.getTimeUs(extractorInput.getPosition());
                if (this.forcedFirstSampleTimestampUs != C0366C.TIME_UNSET) {
    ...
    */
    ...
    private Seeker computeSeeker(ExtractorInput extractorInput) throws IOException {
        ...
        if (maybeReadSeekFrame != null && shouldFallbackToConstantBitrateSeeking(maybeReadSeekFrame) && maybeReadSeekFrame.getDurationUs() != C0366C.TIME_UNSET && (maybeReadSeekFrame.getDataEndPosition() != -1 || extractorInput.getLength() != -1)) {
    ...
    private Seeker maybeReadSeekFrame(ExtractorInput extractorInput) throws IOException {
        ...
        if (extractorInput.getLength() != -1 && parse.dataSize != -1 && extractorInput.getLength() != parse.dataSize + position) {
            Log.m59i(TAG, "Data size mismatch between stream (" + extractorInput.getLength() + ") and Xing frame (" + (parse.dataSize + position) + "), using Xing value.");
        }
        extractorInput.skipFully(this.synchronizedHeader.frameSize);
        if (seekFrameHeader == SEEK_HEADER_XING) {
    ...
    private Seeker getConstantBitrateSeeker(long j, XingFrame xingFrame, long j2) {
        ...
        if (computeDurationUs == C0366C.TIME_UNSET) {
            return null;
        }
    ...
    private static long getId3TlenUs(Metadata metadata) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (entry instanceof TextInformationFrame) {
                ...
                if (textInformationFrame.f120id.equals("TLEN")) {
