...
public final class PesReader implements TsPayloadReader {
    ...
    @Override // androidx.media3.extractor.p001ts.TsPayloadReader
    ...
    public void consume(ParsableByteArray parsableByteArray, int i) throws ParserException {
        ...
        if ((i & 1) != 0) {
            ...
            if (i2 != 0 && i2 != 1) {
                if (i2 == 2) {
                    Log.m61w(TAG, "Unexpected start indicator reading extended header");
                } else if (i2 == 3) {
                    if (this.payloadSize != -1) {
                        Log.m61w(TAG, "Unexpected start indicator: expected " + this.payloadSize + " more bytes");
                    }
    ...
    private boolean parseHeader() {
        ...
        if (readBits != 1) {
            Log.m61w(TAG, "Unexpected start code prefix: " + readBits);
            this.payloadSize = -1;
            return false;
        }
        ...
        if (readBits2 == 0) {
            ...
            this.payloadSize = i;
            if (i < 0) {
                Log.m61w(TAG, "Found negative packet payload size: " + this.payloadSize);
                this.payloadSize = -1;
            }
        }
        return true;
