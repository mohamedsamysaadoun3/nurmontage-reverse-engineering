    ...
    */
    public CuesWithTimingSubtitle(List<CuesWithTiming> list) {
        if (list.size() == 1) {
            ...
            if (cuesWithTiming.durationUs == C0366C.TIME_UNSET) {
                this.eventCues = ImmutableList.m394of(cuesWithTiming.cues);
                this.eventTimesUs = new long[]{normalizeUnsetStartTimeToZero};
                return;
            } else {
                this.eventCues = ImmutableList.m395of((ImmutableList) cuesWithTiming.cues, ImmutableList.m393of());
            ...
            if (i != 0) {
                ...
                if (j2 >= normalizeUnsetStartTimeToZero2) {
                    if (j2 == normalizeUnsetStartTimeToZero2 && ((ImmutableList) arrayList.get(i3)).isEmpty()) {
                        ...
                        Log.m61w(TAG, "Truncating unsupported overlapping cues.");
                        this.eventTimesUs[i3] = normalizeUnsetStartTimeToZero2;
                        arrayList.set(i3, cuesWithTiming2.cues);
                    }
                    if (cuesWithTiming2.durationUs == C0366C.TIME_UNSET) {
                        this.eventTimesUs[i] = j;
                        arrayList.add(ImmutableList.m393of());
                        i++;
                    }
                }
            }
    ...
    public ImmutableList<Cue> getCues(long j) {
        ...
        return binarySearchFloor == -1 ? ImmutableList.m393of() : this.eventCues.get(binarySearchFloor);
    }
}
