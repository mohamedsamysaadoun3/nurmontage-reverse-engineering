...
public final class Cea708Decoder extends CeaDecoder {
            ...
            if (i == 2 || i == 3) {
                if (z) {
                    if (i == 3) {
                        ...
                        if (i3 != -1 && i2 != (i3 + 1) % 4) {
                            resetCueBuilders();
                            Log.m61w(TAG, "Sequence number discontinuity. previous=" + this.previousSequenceNumber + " current=" + i2);
                        }
                        ...
                        if (dtvCcPacket3 == null) {
                            Log.m57e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                        } else {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.m55d(TAG, "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + ");");
        }
        ...
        while (true) {
            ...
            if (readBits == 7) {
                ...
                if (readBits < 7) {
                    Log.m61w(TAG, "Invalid extended service number: " + readBits);
                }
            }
            if (readBits2 == 0) {
                if (readBits != 0) {
                    Log.m61w(TAG, "serviceNumber is non-zero (" + readBits + ") when blockSize is 0");
                }
                    ...
                    if (readBits3 == 16) {
                        ...
                        if (readBits4 <= 31) {
                            ...
                            if (readBits4 <= 127) {
                                ...
                                Log.m61w(TAG, "Invalid extended command: " + readBits4);
                            }
                            z = true;
                        }
                        ...
                        if (readBits3 <= 127) {
                            ...
                            Log.m61w(TAG, "Invalid base command: " + readBits3);
                        }
                        z = true;
                    }
                }
    ...
    private void handleC0Command(int i) {
        if (i != 0) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m61w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.captionChannelPacketData.skipBits(8);
                        break;
                    } else if (i < 24 || i > 31) {
                        Log.m61w(TAG, "Invalid C0 command: " + i);
                        break;
                    } else {
                        Log.m61w(TAG, "Currently unsupported COMMAND_P16 Command: " + i);
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            case 150:
            default:
                Log.m61w(TAG, "Invalid C1 command: " + i);
                break;
                ...
                default:
                    switch (i) {
                        ...
                        case WorkQueueKt.MASK /* 127 */:
                            ...
                            break;
                        default:
                            Log.m61w(TAG, "Invalid G2 character: " + i);
                            break;
                    }
            }
            return;
    ...
    private void handleG3Character(int i) {
        if (i == 160) {
            ...
            Log.m61w(TAG, "Invalid G3 character: " + i);
