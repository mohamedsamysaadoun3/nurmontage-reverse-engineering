...
public final class Tx3gParser implements SubtitleParser {
    ...
    public Tx3gParser(List<byte[]> list) {
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultFontFamily = TX3G_SERIF.equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? C0366C.SERIF_NAME : "sans-serif";
    ...
    public void parse(byte[] bArr, int i, int i2, SubtitleParser.OutputOptions outputOptions, Consumer<CuesWithTiming> consumer) {
        ...
        if (readSubtitleText.isEmpty()) {
            consumer.accept(new CuesWithTiming(ImmutableList.m393of(), C0366C.TIME_UNSET, C0366C.TIME_UNSET));
            return;
        }
        ...
        consumer.accept(new CuesWithTiming(ImmutableList.m394of(new Cue.Builder().setText(spannableStringBuilder).setLine(f, 0).setLineAnchor(0).build()), C0366C.TIME_UNSET, C0366C.TIME_UNSET));
    }
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.m61w(TAG, "Truncating styl end (" + readUnsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            readUnsignedShort2 = spannableStringBuilder.length();
        }
        if (readUnsignedShort >= readUnsignedShort2) {
            Log.m61w(TAG, "Ignoring styl with start (" + readUnsignedShort + ") >= end (" + readUnsignedShort2 + ").");
            return;
        }
